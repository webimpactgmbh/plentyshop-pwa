<template>
  <header ref="referenceRef" :class="headerClass" class="relative w-full md:sticky md:shadow-md z-10">
    <div
      class="flex justify-between items-center flex-wrap md:flex-nowrap px-4 md:px-10 py-2 md:py-5 w-full border-0 bg-white border-neutral-200"
    ...
    <div v-if="viewport.isGreaterOrEquals('lg')">
      <nav ref="floatingRef">
        <ul
          class="flex px-6 py-2 bg-white border-b border-b-neutral-200 border-b-solid justify-center"
          @blur="
            (event) => {

...
export const useUptainData = (context: UptainContext) => {
  ...
  const getCartData = (cartOverride?: Cart | null) => {
    ...
    const netTotalRaw = cartGetters.getBasketAmountNet(cartValue);
    const netTotal = normalizeCurrencyValue(netTotalRaw, cartCurrency, conversionFactor);
  ...
  const getPersonalData = async () => {
    ...
    if (pageType === 'success') {
      ...
      if (!email && !firstname && !lastname) {
        console.log('[Uptain] getPersonalData (success): no personal data available yet');
        return null;
      }

      const personalData = { email, firstname, lastname };
      console.log('[Uptain] getPersonalData (success): transmitting', personalData);
      return personalData;
    }

    const shouldTransmit = await shouldTransmitPersonalData();
    console.log('[Uptain] getPersonalData called:', 'shouldTransmit:', shouldTransmit, 'hasUser:', !!userValue);
    
    if (!shouldTransmit || !userValue) {
      console.log('[Uptain] getPersonalData: returning null', 'shouldTransmit:', shouldTransmit, 'hasUser:', !!userValue);
      return null;
    }

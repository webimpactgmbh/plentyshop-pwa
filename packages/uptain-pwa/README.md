...
```bash
cd apps/web
npm install @uptain-gmbh/uptain-pwa@1.0.5
```

### Turborepo / monorepo (from repository root)

```bash
npm install @uptain-gmbh/uptain-pwa@1.0.5 --workspace=web
# or
cd apps/web && npm install @uptain-gmbh/uptain-pwa@1.0.5
```
...
Copy the Uptain keys from this package’s [`.env.example`](./.env.example) into your shop **`apps/web/.env`**.
...
NUXT_PUBLIC_UPTAIN_CONSENT_MANAGER_VENDOR_ID=
...
|----------|----------|--------|-------------|
| `NUXT_PUBLIC_UPTAIN_ID` | **Yes** (when enabled) | string | Uptain tracking ID from the Uptain dashboard |
| `NUXT_PUBLIC_UPTAIN_ENABLED` | Recommended | `1` / `0` | `1` = load tracking script, `0` = off |
| `NUXT_PUBLIC_UPTAIN_DEBUG_MODE` | No | `1` / `0` | `1` = log `data-*` attributes to the browser console |
| `NUXT_PUBLIC_UPTAIN_COOKIE_GROUP` | No | string | Cookie-bar group key for consent (default: `CookieBar.marketing.label`) |
| `NUXT_PUBLIC_UPTAIN_CONSENT_MANAGER_VENDOR_ID` | No | string | Vendor ID for ConsentManager-specific integrations |
| `NUXT_PUBLIC_UPTAIN_BLOCK_COOKIES_INITIALLY` | No | `true` / `false` | Block Uptain cookies until consent is given |
| `NUXT_PUBLIC_UPTAIN_TRANSMIT_NEWSLETTER_DATA` | No | `true` / `false` | Transmit newsletter-related data |
| `NUXT_PUBLIC_UPTAIN_TRANSMIT_CUSTOMER_DATA` | No | `true` / `false` | Transmit customer-related data |
| `NUXT_PUBLIC_UPTAIN_TRANSMIT_REVENUE` | No | `true` / `false` | Transmit revenue / order value |

**Shop credentials** (`API_ENDPOINT`, `API_SECURITY_TOKEN`, `CONFIG_ID`, …) are **not** part of this package — configure them in the shop `.env` as usual for Plentyshop.

The module merges the Uptain keys into `runtimeConfig.public` automatically. You do **not** need to edit `apps/web/app/configuration/settings.config.ts`.
...
In **`apps/web/.env`** (shop side — not published by this package):
...
| Cookie / consent issues | `NUXT_PUBLIC_UPTAIN_COOKIE_GROUP` matches a group in your cookie bar config; set `NUXT_PUBLIC_UPTAIN_CONSENT_MANAGER_VENDOR_ID` if you use ConsentManager |

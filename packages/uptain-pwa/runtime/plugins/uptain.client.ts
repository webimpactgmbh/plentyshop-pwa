...
import type { Cart, Order } from '@plentymarkets/shop-api';
...
export default defineNuxtPlugin((nuxtApp) => {
  nuxtApp.runWithContext(() => {
    ...
    const { getAllData, shouldBlockCookies, getUptainId, persistSuccessOrderScv, clearPersistedSuccessScv } =
      useUptainData(context);
  ...
  const debugLog = (...args: unknown[]) => {
    if (isSettingEnabled(getDebugMode())) console.log(...args);
  };

  const isProductPage = () => route.meta?.type === 'product' || route.path.includes('/product/');
  ...
  const resetUptainSessionForNewJourney = () => {
    ...
    removeUptainScript();
    debugLog('[Uptain] Cleared __up_sid after success – next script load starts a new session');
  };
  ...
  const logScriptData = (script: HTMLElement | null) => {
    ...
    if (productVariants !== null) {
      debugLog('%c[Uptain] data-product-variants:', 'color: #fbbf24; font-weight: bold;', productVariants || '(empty string)');
    } else {
      debugLog('%c[Uptain] data-product-variants:', 'color: #ef4444; font-weight: bold;', 'NOT SET');
    }
  ...
  const loadUptainScript = async () => {
    debugLog('[Uptain] loadUptainScript called');
    ...
    let product = null;
    if (process.client && isProductPage()) {
        ...
        if (productId) {
          ...
          if (!product || Object.keys(product).length === 0) {
            ...
            while (Date.now() - start < maxWaitMs) {
              ...
              product = getProductFromAllSources();
              if (product && Object.keys(product).length > 0) {
                debugLog('[Uptain] Product available after', Date.now() - start, 'ms, creating script with product data');
                break;
              }
            }
            if (!product || Object.keys(product).length === 0) {
              debugLog('[Uptain] Product not available after', maxWaitMs, 'ms, creating script without product data (will update on frontend:productLoaded)');
              product = null;
            }
          } else {
            debugLog('[Uptain] Product found immediately on initial load');
          }
        }
      }
    }
    ...
    debugLog('[Uptain] getAllData returned:', data ? 'data exists' : 'null', 'hasProductData:', data && Object.keys(data).some(k => k.startsWith('product-')));
    if (!data) {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      if (key === 'product-variants' || key === 'scv') {
        // scv must always be written (incl. "0" / "0.00") so success can overwrite empty-cart fallback
    ...
    debugLog('[Uptain] Script created with', productAttributesInScript, 'product attributes');

    // Append to body
    document.body.appendChild(script);
    debugLog('[Uptain] Script appended to body:', script.src);
    ...
    if (process.client && isProductPage()) {
      ...
      const checkAndUpdateAttributes = async () => {
        ...
        if (checkCount > maxChecks) {
          if (attributeCheckInterval) {
            ...
            debugLog('[Uptain] Stopped attribute check interval after max checks');
          }
          return;
        }
        ...
        if (missingAttributes.length > 0) {
          debugLog('[Uptain] Missing product attributes detected:', missingAttributes);
          ...
          if (product && Object.keys(product).length > 0) {
            debugLog('[Uptain] Product found, updating missing attributes:', missingAttributes);
            ...
            if (productData) {
              ...
              missingAttributes.forEach(attr => {
                ...
                if (value !== undefined && value !== null && value !== '') {
                  ...
                  attributesUpdated++;
                  debugLog(`[Uptain] Updated missing attribute: ${attr} = ${value}`);
                }
              });
              
              if (attributesUpdated > 0) {
                debugLog(`[Uptain] Updated ${attributesUpdated} missing product attributes`);
                ...
                if (stillMissing.length === 0) {
                  if (attributeCheckInterval) {
                    clearInterval(attributeCheckInterval);
                    attributeCheckInterval = null;
                    debugLog('[Uptain] All product attributes are set, stopped check interval');
                  }
                }
              }
            }
      ...
      debugLog('[Uptain] Started attribute check interval for product page');
      ...
      const stopInterval = () => {
        if (attributeCheckInterval) {
          clearInterval(attributeCheckInterval);
          attributeCheckInterval = null;
          debugLog('[Uptain] Stopped attribute check interval on route change');
        }
      };
    ...
    if (process.client && isProductPage() && !hasValidProduct) {
      debugLog('[Uptain] Starting background check for product...', { hasProduct: !!product, productKeys: product ? Object.keys(product).length : 0 });
      const itemId = route.params.itemId as string;
      if (itemId) {
        const { productParams } = createProductParams(route.params);
        const productId = productParams.id.toString();
        if (productId) {
          debugLog('[Uptain] Product not found on initial load, starting background check...');
          ...
          (async () => {
            ...
            while ((Date.now() - startTime) < maxWait) {
              ...
              if (foundProduct && Object.keys(foundProduct).length > 0) {
                ...
                debugLog(`[Uptain] Background check: Product found after ${waitTime}ms, updating script with product data`);
                ...
                if (currentScript) {
                  ...
                  if (productData) {
                    ...
                    debugLog(`[Uptain] Background check: Set ${attributesSet} product attributes`);
                    logScriptData(currentScript);
  ...
  if (process.client) {
    const syncUptainScript = async () => {
      ...
      // Debug logging
      debugLog('[Uptain] Sync - Enabled:', enabled, 'ID set:', !!uptainId, 'Should block:', shouldBlock, 'Has consent:', hasConsent);
    ...
    watch(
      ...
      async (toPath, fromPath) => {
        ...
        if (!shouldBlockCookies() || checkCookieConsent()) {
          ...
          if (isProductPage() && (!product || Object.keys(product).length === 0)) {
            const itemId = route.params.itemId as string;
            if (itemId) {
              const { productParams } = createProductParams(route.params);
              const productId = productParams.id.toString();
    ...
    // orderFromEvent: pass confirmation order explicitly so success scv does not depend on useState timing.
    const refreshScriptDataAndNotifyUptain = async (
      cartFromEvent?: Cart | null,
      orderFromEvent?: Order | null,
    ) => {
      if (!isSettingEnabled(getUptainEnabled()) || !getValidUptainId()) {
        debugLog('[Uptain] refreshScriptDataAndNotifyUptain skip: not enabled or no ID');
        return;
      }
      ...
      if (blockCookies && !hasConsent) {
        debugLog('[Uptain] refreshScriptDataAndNotifyUptain skip: no consent yet', { blockCookies, hasConsent });
        return;
      }
      const currentScript = document.getElementById('__up_data_qp');
      if (!currentScript) {
        debugLog('[Uptain] refreshScriptDataAndNotifyUptain skip: script #__up_data_qp not found');
        return;
      }
      const product = getProductFromState();
      const data = await getAllData(product, cartFromEvent ?? undefined, orderFromEvent ?? undefined);
      if (!data) {
        debugLog('[Uptain] refreshScriptDataAndNotifyUptain skip: getAllData returned null');
        return;
      }
      if (data.products !== undefined) {
        const preview = String(data.products);
        debugLog('[Uptain] refreshScriptDataAndNotifyUptain setting data-products, length:', preview.length, 'preview:', preview.length > 80 ? preview.slice(0, 80) + '...' : preview);
      }
      const alwaysPresentAttributes = ['plugin', 'returnurl', 'page', 'wishlist'];
      const personalDataFields = ['email', 'firstname', 'lastname', 'gender', 'title', 'uid', 'revenue', 'customergroup'];
      const existingAttributes = Array.from(currentScript.getAttributeNames())
    ...
    if (process.client) {
      ...
      // When confirmation order loads (async), refresh so email/firstname/lastname + scv are sent
      watch(
        () => confirmationOrderState.value?.data,
        (order) => {
          if (!order || !route.path.includes('/confirmation/')) return;
          persistSuccessOrderScv(order as Order);
          void refreshScriptDataAndNotifyUptain(undefined, order as Order);
        },
      );

      // Persist scv before navigation to confirmation (survives window.location.assign)
      onPlentyEvent('frontend:orderCreated', (order: Order) => {
      ...
      const getCartSignature = (cart: Cart | undefined) => {
        ...
        const basketNet = cartGetters.getItemSumNet(cart) || 0;
      ...
      const updateProductDataInScript = async (product: any) => {
        ...
        debugLog('[Uptain] updateProductDataInScript called with product:', product);
        const currentScript = document.getElementById('__up_data_qp');
        if (currentScript) {
          const data = await getAllData(product);
          debugLog('[Uptain] getAllData returned:', data);
          if (data) {
            ...
            Object.entries(data).forEach(([key, value]) => {
              if (key.startsWith('product-') || key === 'product') {
                ...
                if (key === 'product-variants') {
                  currentScript.setAttribute(`data-${key}`, String(value || ''));
                  debugLog('[Uptain] Set product attribute:', key, value || '');
            ...
            debugLog('[Uptain] Total product attributes set:', productAttributesSet);
      ...
      onPlentyEvent('frontend:productLoaded', async (eventData: { product: any }) => {
        ...
        debugLog('[Uptain] frontend:productLoaded event received, updating script data', product);
        ...
        if (itemId) {
          ...
          if (productId) {
            ...
            watch(
              ...
              async (product) => {
                if (product && Object.keys(product).length > 0) {
                  debugLog('[Uptain] currentProduct changed, updating script');
                  await updateProductDataInScript(product);
                }
              },
            ...
            watch(
              ...
              async (product) => {
                if (product && Object.keys(product).length > 0) {
                  debugLog('[Uptain] productForEditor changed, updating script');
                  await updateProductDataInScript(product);
                }
              },
              { deep: true, immediate: false },
            ...
            watch(
              ...
              async (product) => {
                if (product && Object.keys(product).length > 0) {
                  debugLog('[Uptain] productData changed, updating script');
                  await updateProductDataInScript(product);
                }
              },
              { deep: true, immediate: false },
            ...
            const checkAndUpdateProduct = async () => {
              ...
              if (product && Object.keys(product).length > 0) {
                ...
                if (productHash !== lastProductHash) {
                  debugLog('[Uptain] Continuous check: Product found, updating script');
            ...
            setTimeout(() => {
              if (checkInterval) {
                clearInterval(checkInterval);
                checkInterval = null;
                debugLog('[Uptain] Stopped continuous product check after 10 seconds');
              }
            }, 10000);
      ...
      onPlentyEvent('frontend:addToWishlist', async () => {
        ...
        debugLog('[Uptain] frontend:addToWishlist event received, updating wishlist data');
        const currentScript = document.getElementById('__up_data_qp');
        if (currentScript) {
          ...
          if (data && data.wishlist) {
            ...
            debugLog('[Uptain] Updated wishlist data:', data.wishlist);
      ...
      const updateWishlistInScript = async () => {
        ...
        if (currentScript) {
          ...
          if (data && data.wishlist) {
            currentScript.setAttribute('data-wishlist', data.wishlist);
            debugLog('[Uptain] Updated wishlist in script:', data.wishlist);
      ...
      watch(
        ...
        async (wishlistData) => {
          if (wishlistData && wishlistData.length > 0) {
            debugLog('[Uptain] Wishlist data changed in state, length:', wishlistData.length);
      ...
      watch(
        ...
        async (ids) => {
          if (ids && ids.length > 0) {
            debugLog('[Uptain] Wishlist IDs changed:', ids);
